...
def run_mcocr_pipeline(
    ...
    *,
    ...
    # Auto-correct labeled boxes if final_amount was found via surrounding scans (ocr_total)
    # but not tagged as TOTAL_COST in labeled_boxes
    final_amount = summary.get("amount")
    if final_amount:
        from .receipt_nlu import _parse_vn_amount, _amounts_in_line
        
        has_total_cost_box = any(
            b.get("entity") == "TOTAL_COST" and 
            (_parse_vn_amount(b.get("text", "")) == final_amount or final_amount in _amounts_in_line(b.get("text", "")))
            for b in kie.labeled_boxes
        )
        if not has_total_cost_box:
            for b in kie.labeled_boxes:
                if b.get("entity") == "OTHER" or b.get("entity") == "OTHER_VALUE":
                    val = _parse_vn_amount(b.get("text", ""))
                    amounts = _amounts_in_line(b.get("text", ""))
                    if val == final_amount or final_amount in amounts:
                        b["entity"] = "TOTAL_COST"
