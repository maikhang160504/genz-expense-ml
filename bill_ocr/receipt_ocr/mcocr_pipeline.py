...
from .paddle_compat import paddle_lines
...
class McOcrReceiptPipeline(ReceiptOCRPipeline):
    ...
    def __init__(
        ...
        layoutlmv3_model: str | Path | None = None,
    ...
    def _get_kie(self):
        if self._kie is None:
            self._kie = get_kie_engine(model_path=self.layoutlmv3_model, device=self.device)
        return self._kie
    ...
    def ocr_boxes(self, image_rgb: np.ndarray):
        h_orig, w_orig = image_rgb.shape[:2]
        forward_mats: list[Any] = []
        working = image_rgb
        paddle_cache = None
        rotator = self._get_rotator()
        if self.use_rotation and rotator.available:
            paddle_cache = self._run_paddle(image_rgb)
            polys = [line[0] for line in paddle_lines(paddle_cache)]
            if polys:
                working, rot_meta = rotator.correct(image_rgb, polys)
                forward_mats = list(rot_meta.get("forward_matrices") or [])
                if rot_meta.get("applied"):
                    paddle_cache = None

        if paddle_cache is not None and working is image_rgb:
            df = self._boxes_from_paddle_result(image_rgb, paddle_cache)
        else:
            df = super().ocr_boxes(working)

        if forward_mats and df is not None and not df.empty:
            from .mcocr_rotation.geometry import map_boxes_df_to_original

            df = map_boxes_df_to_original(df, forward_mats, w_orig, h_orig)
        return df
    ...
    def prelabel_for_admin(self, image_rgb: np.ndarray) -> dict[str, Any]:
        """Auto-label for WebAdmin: OCR → LayoutLMv3 entities → fusion summary."""
        result = self.process_image_rgb(image_rgb, split_mode=False)
        kie = self._get_kie()
        warnings = list(result.get("warnings") or [])
        if kie.backend != "layoutlmv3":
            msg = getattr(kie, "_load_error", None) or "LayoutLMv3 weights not loaded — using heuristic KIE."
            warnings.append(msg)
        return {
            ...
            "auto_label_engine": "paddle+rotation+vietocr+layoutlmv3"
            if result.get("kie_backend") == "layoutlmv3"
