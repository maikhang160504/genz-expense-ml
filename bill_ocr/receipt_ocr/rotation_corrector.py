...
class RotationCorrector:
    ...
    def correct(self, image_rgb: np.ndarray, polys: list[Any]) -> tuple[np.ndarray, dict[str, Any]]:
        ...
        forward_matrices: list[Any] = []
        img_rotated, boxes_list, mat_skew = rotate_image_bbox_angle(image_bgr, boxes_list, skew)
        forward_matrices.append(mat_skew)
        flip = int(vote_page_flip(self._model, img_rotated, boxes_list))
        if flip:
            img_rotated, boxes_list, mat_flip = rotate_image_bbox_angle(img_rotated, boxes_list, flip)
            forward_matrices.append(mat_flip)
