...
def rotate_image_bbox_angle(
    img: np.ndarray, bboxes: list[Any], angle: float
    ...
    return img_result, ret_boxes, mat


def inverse_map_rect(x1: int, y1: int, x2: int, y2: int, mat: np.ndarray) -> tuple[int, int, int, int]:
    """Map axis-aligned rect from warped image back through inverse affine ``mat``."""
    ...
    warped = (mat_inv @ np.hstack([corners, ones]).T).T
    xs, ys = warped[:, 0], warped[:, 1]
    return int(round(float(xs.min()))), int(round(float(ys.min()))), int(round(float(xs.max()))), int(round(float(ys.max())))


def map_boxes_df_to_original(
    df_boxes: "pd.DataFrame",
    forward_matrices: list[np.ndarray],
    img_w: int,
    img_h: int,
) -> "pd.DataFrame":
    """Re-map OCR boxes from rotation-corrected space to original image coordinates."""
    import pandas as pd

    if df_boxes is None or df_boxes.empty or not forward_matrices:
        return df_boxes
    out = df_boxes.copy()
    for idx in out.index:
        x1, y1, x2, y2 = int(out.at[idx, "x1"]), int(out.at[idx, "y1"]), int(out.at[idx, "x2"]), int(out.at[idx, "y2"])
        for mat in reversed(forward_matrices):
            x1, y1, x2, y2 = inverse_map_rect(x1, y1, x2, y2, mat)
        out.at[idx, "x1"] = max(0, min(x1, img_w - 1))
        out.at[idx, "y1"] = max(0, min(y1, img_h - 1))
        out.at[idx, "x2"] = max(out.at[idx, "x1"] + 1, min(x2, img_w))
        out.at[idx, "y2"] = max(out.at[idx, "y1"] + 1, min(y2, img_h))
    return out
