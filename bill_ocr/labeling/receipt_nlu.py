...
_TOTAL_LINE_RULES: tuple[tuple[int, re.Pattern[str]], ...] = (
    ...
    (96, re.compile(r"tiền\s*thanh\s*toán|tien\s*thanh\s*toan", re.I)),
...
_RE_AMOUNT_ONLY_LINE = re.compile(r"^[\d\s.,+\-/%]+$")
_RE_BARCODE_LINE = re.compile(r"^\d{8,}$")
_RE_QTY_PRICE_LINE = re.compile(r"^\s*\d+\s+[\d.,]+\s+[\d.,]+\s*$")
_RE_TABLE_HEADER = re.compile(
    r"mat\s*hang|mặt\s*hàng|don\s*gia|đơn\s*giá|\bsl\b|t\.?\s*tien|thanh\s*tien|ttien",
    re.I,
)
_RE_DISCOUNT_LINE = re.compile(
    r"da\s*giam|đã\s*giảm|giam\s*gia|giảm\s*giá|chiet\s*khau|chiết\s*khấu",
    re.I,
)
...
def filter_lines_for_category(lines: list[str]) -> list[str]:
    """Drop receipt boilerplate / numeric rows — keeps product-like text (fallback)."""
    kept: list[str] = []
    for raw in lines:
        line = str(raw).strip()
        if len(line) < 3:
        ...
        if _SKIP_LINE.search(line):
            continue
        if _line_total_score(line) >= 70:
            continue
        compact = line.replace(" ", "")
        if _RE_AMOUNT_ONLY_LINE.match(compact):
            continue
        if _RE_BARCODE_LINE.match(compact):
            continue
        if _RE_QTY_PRICE_LINE.match(line):
            continue
        if not re.search(r"[a-zA-Zà-ỹÀ-Ỹ]", line):
            continue
        kept.append(line)
    return kept


def _clean_product_name_line(line: str) -> str | None:
    """Normalize a single OCR line to a product name candidate."""
    s = str(line).strip()
    if len(s) < 3:
        return None
    if _HEADER_SKIP.search(s) or _SKIP_LINE.search(s):
        return None
    if _RE_TABLE_HEADER.search(s):
        return None
    if _line_total_score(s) >= 70 or _RE_DISCOUNT_LINE.search(_normalize(s)):
        return None
    compact = s.replace(" ", "")
    if _RE_AMOUNT_ONLY_LINE.match(compact) or _RE_BARCODE_LINE.match(compact):
        return None
    if _RE_QTY_PRICE_LINE.match(s):
        return None
    cleaned = re.sub(r"^\s*\d+[\s.\-]+", "", s).strip()
    cleaned = re.sub(r"\s+\d{1,3}(?:[.,]\d{3})+\s*$", "", cleaned).strip()
    if len(cleaned) < 3 or not re.search(r"[a-zA-Zà-ỹÀ-Ỹ]{2,}", cleaned):
        return None
    if _amounts_in_line(cleaned) and not re.search(r"[a-zA-Zà-ỹÀ-Ỹ]{4,}", cleaned):
        return None
    return cleaned


def extract_product_names_from_lines(lines: list[str]) -> list[str]:
    """Lấy tên mặt hàng giữa header bảng và dòng tổng tiền."""
    products: list[str] = []
    in_table = False
    for raw in lines:
        line = str(raw).strip()
        if not line:
            continue
        if _RE_TABLE_HEADER.search(line):
            in_table = True
            continue
        if in_table and _line_total_score(line) >= 70:
            break
        if not in_table:
            continue
        name = _clean_product_name_line(line)
        if name:
    ...
    if n <= 4:
        return []
    start = min(4, max(1, n // 5))
    end = max(start + 1, n - max(2, n // 4))
    for raw in lines[start:end]:
        name = _clean_product_name_line(str(raw))
        if name:
            products.append(name)
    return products


def extract_product_names_for_category(
    lines: list[str],
    matched_items: list[tuple[str, int]] | None = None,
) -> list[str]:
    """Chỉ đưa tên sản phẩm vào category model — ưu tiên ghép bbox name/price."""
    if matched_items:
        from_boxes = [
            re.sub(r"^\s*\d+[\s.\-]+", "", str(name).strip()).strip()
            for name, _ in matched_items
            if name and len(str(name).strip()) >= 2
        ]
        from_boxes = [n for n in from_boxes if _clean_product_name_line(n)]
        if from_boxes:
            return from_boxes
    from_lines = extract_product_names_from_lines(lines)
    if from_lines:
        return from_lines
    return filter_lines_for_category(lines)


def _predict_category_model(
    lines: list[str],
    *,
    product_names: list[str] | None = None,
) -> str | None:
    """Gọi category model NLU trên tên sản phẩm (không phải toàn bộ OCR lines)."""
    try:
        _ensure_train_on_path()
        ...
        if product_names:
            ...
            source = extract_product_names_for_category(lines)
        if not source:
            source = [ln for ln in lines if len(str(ln).strip()) >= 3][:6]
        text = clean_category_text(" ".join(source)[:600])
        if not text.strip():
            return None
...
def classify_category(
    ...
    Chỉ dùng tên sản phẩm (bbox alignment hoặc trích từ vùng bảng hàng).
    """
    product_names = extract_product_names_for_category(lines, matched_items)
    score_lines = product_names if product_names else lines
    scores = _score_categories_from_lines(score_lines)
    ...
    model_cat = _predict_category_model(lines, product_names=product_names or None)
...
def align_skewed_items_and_prices(
    ...
    for _, row in df_boxes.iterrows():
        ...
        if amounts and x1 >= boundary_x:
            ...
            name = _clean_product_name_line(cleaned_text) if cleaned_text else None
            if name:
                names.append({"text": name, "bbox": [x1, y1, x2, y2]})
...
def predict_item_category(item_name: str) -> tuple[str, float]:
    ...
    model_cat = _predict_category_model([], product_names=[item_name])
    if model_cat:
