"""
PhoBERT embedding + logistic income/expense.
...
from encoder_data import load_record_type_rows, xy_from_df  # noqa: E402
...
OUT_PATH = Path(
    os.environ.get("RECORD_TYPE_ENCODER_OUT", str(ROOT / "models" / "record_type_encoder.joblib"))
)


def main() -> None:
    model_name = os.environ.get("ENCODER_MODEL_NAME", "vinai/phobert-base")
    df = load_record_type_rows()
    print(f"Record-type rows: {len(df)}  classes={sorted(df['type'].unique())}")

    texts, y = xy_from_df(df, "type")
    ...
    save_task_metrics("record_type", compute_sklearn_metrics(y_test, y_pred))
